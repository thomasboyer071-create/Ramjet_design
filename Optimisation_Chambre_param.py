...
Acol_grid = np.zeros((n_pts, n_pts))   # Taille Col
...
p02 = p1_amb * (1 + 0.2 * 2.7**2)**3.5 * 0.85 # eta_diff approx
...
axs[0, 0].set_title(r'Température de sortie $T_{03}$ (K)')
plt.colorbar(c1, ax=axs[0, 0])

# M3
c2 = axs[0, 1].contourf(PHI, M2, M3_grid, 20, cmap='jet')
axs[0, 1].set_title(r'Mach de sortie Chambre $M_3$')
plt.colorbar(c2, ax=axs[0, 1])

# A2 (Chambre)
c3 = axs[1, 0].contourf(PHI, M2, A2_grid/b, 20, cmap='viridis')
axs[1, 0].set_title(r'Hauteur $A_2$ (m)')
plt.colorbar(c3, ax=axs[1, 0])

# Acol (Col)
c4 = axs[1, 1].contourf(PHI, M2, Acol_grid/b, 20, cmap='plasma')
axs[1, 1].set_title(r'Hauteur Col Tuyère $A_{col}$ (m)')
